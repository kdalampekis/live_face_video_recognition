...
Face_Images = os.path.join(os.getcwd(), "/Users/kostasbekis/live_face_recognition/photos")  # Tell the program where we have saved the face images
print(Face_Images)
...
for root, dirs, files in os.walk(Face_Images):  # go to the face image directory
    for file in files:  # check every directory in it
        if file.endswith("jpeg") or file.endswith("jpg") or file.endswith(
                "png"):  # for image files ending with jpeg,jpg or png
            ...
            Crop_Image = Gery_Image.resize((800, 800),
                                           Image.ANTIALIAS)  # Crop the Grey Image to 550*550 (Make sure your face is in the center in all image)
